...
void MPlayer::setupTextStyle(sf::Text& txt, const std::string& str, sf::Color color, float x, float y) {
    txt.setFont(font);
...
std::string MPlayer::Convert_MP4_To_Wave(std::string& filepath) {
    ...
    if (result != 0) {
        std::cerr << "Warning: ffmpeg conversion returned non-zero: " << result << std::endl;
    }
    return wavFile;
}

float MPlayer::Get_Volume() {
