...
static bool write(struct TinyPngOut this[static 1], const uint8_t data[],
                  size_t len);
static void crc32(struct TinyPngOut this[static 1], const uint8_t data[],
                  size_t len);
static void adler32(struct TinyPngOut this[static 1], const uint8_t data[],
                    size_t len);
...
enum TinyPngOut_Status TinyPngOut_write(struct TinyPngOut this[static 1],
  ...
  while (count > 0) {
    ...
    assert(this->positionX < this->lineSize &&
           this->deflateFilled < DEFLATE_MAX_BLOCK_SIZE);
